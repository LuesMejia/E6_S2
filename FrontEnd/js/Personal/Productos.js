...
const renderProducts = (productosLista) => {
    ...
    if (!Array.isArray(productosLista) || productosLista.length === 0) {
        elements.productosTabla.innerHTML = '<tr><td colspan="5">No se encontraron productos</td></tr>';
        return;
    }
    ...
    productosLista.forEach(producto => {
        ...
        row.innerHTML = `
         <td>${producto.foto_producto}</td>
            ...
            <td>
                <button class="btn btn-sm btn-warning edit-btn" data-id="${producto._id}"><i class="fa fa-edit"></i> Editar</button>
                <button class="btn btn-sm btn-danger delete-btn" data-id="${producto._id}"><i class="fa fa-trash"></i> Eliminar</button>
            </td>
        `;
...
const actualizarPoducto = async () => {
    ...
    formData.append("precio", parseFloat(document.getElementById('updatePrecio').value));
    formData.append("estado", document.getElementById('updateEstado').value);
    formData.append("categoria", document.getElementById('updateCategoria').value);
...
const filterProducts = () => {
    ...
    const filtered = producto.filter(p =>
...
const setupEventListeners = () => {
    ...
    document.getElementById("gurdarProductoBtn").addEventListener("click", crearProductos);
    document.getElementById("actualizarProductBtn").addEventListener("click", actualizarPoducto);
